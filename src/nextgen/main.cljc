(ns nextgen.main
  "Kotodama WASM entrypoint for the nextgen clean-room actor (L4) — Clojure port.
  ...
  facts (`nextgen.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nextgen")
(def tier "L4")
...
(def entity-specs
  [{:entity "Patient"   :plural "patients"   :id-prefix "nextgen_pat"
   ...
   {:entity "Observation" :plural "observations" :id-prefix "nextgen_obs"
    :fields [:resourceType :status :effectiveDateTime :valueString :issued] :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "Encounter" :plural "encounters" :id-prefix "nextgen_enc"
    :fields [:resourceType :status :serviceType :period] :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "MedicationRequest" :plural "medicationrequests" :id-prefix "nextgen_med"
    :fields [:resourceType :status :intent :authoredOn] :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "AllergyIntolerance" :plural "allergyintolerances" :id-prefix "nextgen_all"
    :fields [:resourceType :clinicalStatus :criticality :recordedDate] :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "Condition" :plural "conditions" :id-prefix "nextgen_con"
...
(defn emit-facts
  "EAVT facts for one record: {\"nextgen.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nextgen-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nextgen-compat/ready)
